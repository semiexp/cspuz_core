...
use cspuz_rs_puzzles::puzzles::pentominous;
...
pub fn solve(url: &str) -> Result<Board, &'static str> {
    let (clues, default_borders) =
        pentominous::deserialize_pentominous_problem(url).ok_or("invalid url")?;
    let border = pentominous::solve_pentominous(&clues, &default_borders).ok_or("no answer")?;
