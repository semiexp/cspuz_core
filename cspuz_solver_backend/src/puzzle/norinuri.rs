...
use cspuz_rs_puzzles::puzzles::norinuri;

pub fn solve(url: &str) -> Result<Board, &'static str> {
    let problem = norinuri::deserialize_problem(url).ok_or("invalid url")?;
    let ans = norinuri::solve_norinuri(&problem).ok_or("no answer")?;
...
mod tests {
    ...
    fn test_solve() {
        compare_board!(
            solve("https://puzz.link/p?norinuri/6/6/g4i.i2v.i.i.g"),
            Board {
                kind: BoardKind::Grid,
                height: 6,
                width: 6,
                data: vec![
                    ...
                    Item { y: 9, x: 5, color: "black", kind: ItemKind::Text("?") },
                    ...
                    Item { y: 11, x: 1, color: "black", kind: ItemKind::Text("?") },
