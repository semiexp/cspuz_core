...
use cspuz_rs_puzzles::puzzles::kurodoko;

pub fn solve(url: &str) -> Result<Board, &'static str> {
    let problem = kurodoko::deserialize_problem(url).ok_or("invalid url")?;
    let ans = kurodoko::solve_kurodoko(&problem).ok_or("no answer")?;
...
mod tests {
    ...
    fn test_solve() {
        compare_board!(
            solve("https://puzz.link/p?kurodoko/6/5/h2j2h3r5g2j"),
            Board {
                kind: BoardKind::Grid,
                height: 5,
                width: 6,
                data: vec![
                    ...
                    Item { y: 3, x: 9, color: "black", kind: ItemKind::Num(3) },
                    ...
                    Item { y: 7, x: 11, color: "black", kind: ItemKind::Num(5) },
                    Item { y: 9, x: 1, color: "green", kind: ItemKind::Fill },
                    Item { y: 9, x: 3, color: "black", kind: ItemKind::Circle },
                    Item { y: 9, x: 3, color: "black", kind: ItemKind::Num(2) },
